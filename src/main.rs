...
fn feed_forward(input:&Vec<f64>, weights:&Vec<f64>, biases:&Vec<f64>) -> f64{
    let hidden1 = sigmoid(weights[0] * input.get(0).unwrap() + weights[1] * input.get(1).unwrap() + biases[0]);
    let hidden2 = sigmoid(weights[2] * input.get(0).unwrap() + weights[3] * input.get(1).unwrap() + biases[1]);
...
fn train(train_inpt:&Vec<Vec<f64>>, train_out:&Vec<f64>, weights:&mut Vec<f64>, biases:&mut Vec<f64>, learning_rate:f64, epochs:i32){
    for _i in 0..epochs{
        for x in 0..train_out.len(){
            let input :&Vec<f64> = train_inpt.get(x).unwrap();
            let sum_hidden1 = weights[0] * input.get(0).unwrap() + weights[1] * input.get(1).unwrap() + biases[0];
            let hidden1 = sigmoid(sum_hidden1);
            let sum_hidden2 = weights[2] * input.get(0).unwrap() + weights[3] * input.get(1).unwrap() + biases[1];
            let hidden2= sigmoid(sum_hidden2);
            let sum_output = weights[4] * hidden1 + weights[5] * hidden2 + biases[2];
            let output = sigmoid(sum_output);
            let d_l_d_out = -2_f64 * (train_out.get(x).unwrap() - output);
            ...
            let d_h1_d_w0 = input.get(0).unwrap() * deriv_sigmoid(sum_hidden1);
            let d_h1_d_w1 = input.get(1).unwrap() * deriv_sigmoid(sum_hidden1);
            let d_h1_d_b0 = deriv_sigmoid(sum_hidden1);
            let d_h2_d_w2 = input.get(0).unwrap() * deriv_sigmoid(sum_hidden2);
            let d_h2_d_w3 = input.get(1).unwrap() * deriv_sigmoid(sum_hidden2);
